...
import { findApptById, loadAppts, saveAppts } from "../../data/doctor-appts.store.js";
...
export default function ReceptionAppointmentPay() {
    ...
    function onProceed() {
        ...
        const idx = list.findIndex((r) => r.id === appt.id);
        if (idx !== -1) {
            list[idx] = { ...list[idx], status: "completed" };
            saveAppts(list);
        }
        nav(`/reception/appointments/completed/${encodeURIComponent(appt.id)}`, {
            state: { appt: { ...appt, status: "completed" } },
        });
    }
                        ...
                        <div className="grid h-full min-h-0 gap-6 lg:grid-cols-[340px_minmax(0,1fr)_360px]">
                            ...
                            <section className="rounded-3xl bg-white/6 border border-white/10 p-6 flex flex-col min-h-0">
                                ...
                                <div className="mt-5 flex justify-end">
                                    <button
                                        onClick={onProceed}
                                        className="rounded-xl bg-[#0E1A33] text-white px-8 py-3 font-medium hover:opacity-95"
                                    >
                                        Proceed
                                    </button>
                                </div>
