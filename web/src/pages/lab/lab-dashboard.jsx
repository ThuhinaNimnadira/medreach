...
export default function LabDashboard() {
    const reports = React.useMemo(() => loadReports(), []);
    const issued = React.useMemo(
        () => reports.filter(r => r.status === "issued").sort((a,b)=> (b.issuedAt||0)-(a.issuedAt||0)).slice(0,10),
        [reports]
    );
    const SUMMARY = { reports: reports.length, notifications: 12 };
      ...
      `}</style>
            ...
            <header className="flex justify-between items-center px-12 py-4  shrink-0">
                ...
                <nav className="flex items-center gap-1 text-[#ffffff] font-medium">
                    ...
                    <Link to="/lab/settings" className="w-24 h-10 flex items-center justify-center px-4 py-2 rounded-md hover:bg-[#40E0D0] hover:text-white hover:scale-105 transition-all">Settings</Link>
            ...
            <main className="flex flex-1 min-h-0 px-12 py-10 gap-8">
                <aside className="w-[255px] h-[273px]">
                    <h1 className="text-2xl font-bold mb-2">Hello,<br/><span className="text-[#ffffff] text-4xl">Username!</span></h1>
                    ...
                    <div className="bg-white rounded-xl shadow-md p-10 space-y-2 h-full">
                        ...
                        <p className="text-gray-700"><span className="font-semibold">{SUMMARY.reports}</span> Reports</p>
                        <p className="text-green-600"><span className="font-semibold">{SUMMARY.notifications}</span> Notifications</p>
                    </div>
                </aside>

                <section className="flex-1 grid grid-cols-2 gap-8 min-h-0">
                    <div className="bg-white rounded-xl shadow-md p-10 flex flex-col justify-between w-[439px] h-[400px] hover:scale-105
                      duration-300">
                        <div>
                            ...
                            <h3 className="text-3xl font-bold mb-4 mt-4 text-[#0C164F]">Reports</h3>
                            <p className="text-gray-500 mb-6">Access all new &amp; issued reports.</p>
                        </div>
                        <Link to="/lab/reports" className="text-gray-600 font-semibold hover:text-[#0C164F]">View &raquo;</Link>
                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-md p-10 w-[439px] h-[400px] flex flex-col overflow-hidden">
                        <div className="shrink-0">
                            ...
                            <p className="text-gray-500">recently issued reports.</p>
                        </div>
                        <div className="docs-scroll flex-1 overflow-y-auto mt-6 space-y-4 pr-2">
                            {issued.map((r) => (
                                <div key={r.id} className="flex items-center gap-3 bg-[#EEF0FF] rounded-2xl px-4 py-4">
                                    <div className="grid place-items-center w-12 h-12 bg-white/70 rounded-lg"><ClipboardIcon className="w-7 h-7" /></div>
