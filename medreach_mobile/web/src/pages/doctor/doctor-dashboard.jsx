...
export default function DoctorDashboard() {
    ...
    const [summary, setSummary] = useState({ reviews: 0, notifications: 0 });
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            ...
            try {
                ...
                setUpcomingCount(upcomingAppts.length);

                setSummary({
                    reviews: 0, // can be fetched from reviews collection later
                    notifications: 0, // can be fetched from notifications collection
                });
            } catch (e) {
    ...
    if (loading) {
        return (
            <div className="h-screen w-screen flex items-center justify-center bg-[#F5F6F8] text-[#0C164F] text-xl font-semibold">
                Loading Dashboard...
            </div>
        );
    }
            ...
            <header className="flex justify-between items-center px-12 py-4 shrink-0">
                ...
                <div className="flex items-center gap-6">
                    <nav className="flex items-center gap-1 text-[#FFFFFF] font-medium">
                        <Link to="/doctor/notifications" className="mt-1 pr-4" title="Notifications">
                            <Bell className="w-5 h-5 text-w cursor-pointer bell-hover" />
                        </Link>
                        <Link
                            to="/doctor/appointments"
                            className="w-32 h-10 flex items-center justify-center px-4 py-2 rounded-md hover:bg-[#40E0D0] hover:text-white hover:scale-105 transition-all duration-300"
                        >
                            Appointments
                        </Link>
                        <Link
            ...
            <main className="flex flex-1 min-h-0 px-12 py-10 gap-8 pt-15">
                ...
                <aside className="w-[255px] h-[273px]">
                    <h1 className="text-2xl text-white font-bold mb-2">
                        Hello,
                        <br />
                        <span className="text-[#ffffff] text-4xl">Doctor!</span>
                    </h1>
                    <p className="text-sm text-white mb-6">Stay In Control, Stay Connected.</p>

                    <div className="bg-white rounded-xl shadow-md p-10 space-y-2 h-full backdrop-blur-lg">
                        <h2 className="text-2xl font-semibold text-gray-800 pb-3">What’s <br />New?</h2>
                        <p className="text-gray-700">
                            <span className="font-semibold">{upcomingCount}</span> Appointments
                        </p>
                        <p className="text-green-600">
                            <span className="font-semibold">{summary.reviews}</span> New Reviews
                        </p>
                ...
                <section className="flex-1 grid grid-cols-2 gap-8 min-h-0">
                    ...
                    <div className="bg-white rounded-xl shadow-md p-10 flex flex-col justify-between w-[439px] h-[400px] hover:text-white hover:scale-105 transition-all duration-300">
                        <div>
                            ...
                            <h3 className="text-3xl font-bold mb-4 mt-4 text-[#0C164F]">My appointments</h3>
                            <p className="text-gray-500 mb-6">
                                Access all upcoming &amp; completed
                                <br />
                                appointments.
                            </p>
                        </div>
                        <Link to="/doctor/appointments" className="text-gray-600 font-semibold hover:text-[#0C164F]">
                            View &raquo;
                        </Link>
                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-md p-10 w-[439px] h-[400px] flex flex-col overflow-hidden">
                        <div className="shrink-0">
                            ...
                            <p className="text-gray-500">recent completed appointments.</p>
                        </div>

                        <div className="docs-scroll flex-1 overflow-y-auto mt-6 space-y-4 pr-2">
                            {completed.map((r) => (
                                <div key={r.id} className="flex items-center gap-3 bg-[#EEF0FF] rounded-2xl px-4 py-4">
                                    <div className="grid place-items-center w-12 h-12 bg-white/70 rounded-lg">
                                        <ClipboardIcon className="w-7 h-7" />
                                    </div>

                                    <div className="flex-1">
                                        <div className="text-[15px] font-semibold text-[#0C164F]">{r.patient}</div>
                                        <div className="text-sm text-gray-600">{r.date}</div>
                                    </div>

                                    <Link
                                        to={`/doctor/appointments/completed/${r.id}`}
                                        state={{ appt: r }}
                                        onClick={() => sessionStorage.setItem("lastAppt", JSON.stringify(r))}
